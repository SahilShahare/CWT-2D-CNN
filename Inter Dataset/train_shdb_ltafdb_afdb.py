...
os.makedirs('./logs/shdb_ltafdb_afdb', exist_ok=True)

now = datetime.now()
timestamp = now.strftime("%Y-%m-%d_%H:%M:%S")

log_file = f"./logs/shdb_ltafdb_afdb/{timestamp}.txt"
...
X_train = np.concatenate([np.load(X_path_shdb),np.load(X_path_ltafdb)])
Y_train = np.concatenate([np.load(Y_path_shdb),np.load(Y_path_ltafdb)])
...
X_test = np.load(X_path_afdb)
Y_test = np.load(Y_path_afdb)
