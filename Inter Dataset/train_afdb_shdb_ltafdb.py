...
os.makedirs('./logs/afdb_shdb_ltafdb', exist_ok=True)

now = datetime.now()
timestamp = now.strftime("%Y-%m-%d_%H:%M:%S")

log_file = f"./logs/afdb_shdb_ltafdb/{timestamp}.txt"
...
X_train = np.concatenate([np.load(X_path_afdb),np.load(X_path_shdb)])
Y_train = np.concatenate([np.load(Y_path_afdb),np.load(Y_path_shdb)])
...
X_test = np.load(X_path_ltafdb)
Y_test = np.load(Y_path_ltafdb)
